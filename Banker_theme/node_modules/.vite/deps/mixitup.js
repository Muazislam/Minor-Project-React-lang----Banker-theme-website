...
//#region node_modules/mixitup/dist/mixitup.js
var require_mixitup = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	/**!
	* MixItUp v3.3.2
	...
	(function(window) {
		...
		(function() {
			var VENDORS = [
				...
				"o",
				"ms"
			], canary = window.document.createElement("div"), i = -1;
			...
			if (typeof canary.nextElementSibling === "undefined") Object.defineProperty(window.Element.prototype, "nextElementSibling", { get: function() {
		...
		*/
		mixitup = function(container, config, foreignDoc) {
			...
			else if (container && typeof container === "object" && h.isElement(container, doc)) elements = [container];
			else if (container && typeof container === "object" && container.length) elements = container;
		...
		h = {
			...
			extend: function(destination, source, deep, handleErrors) {
				...
				try {
					...
					for (i = 0; i < sourceKeys.length; i++) {
						key = sourceKeys[i];
						if (!deep || typeof source[key] !== "object" || this.isElement(source[key])) destination[key] = source[key];
			...
			*/
			handleExtendError: function(err, destination) {
				...
				if (err instanceof TypeError && (matches = re.exec(err.message))) {
					...
					if (mostMatchingChars > 1) suggestion = mixitup.messages.errorConfigInvalidPropertySuggestion({ probableMatch });
			...
			*/
			isElement: function(el, doc) {
				doc = doc || window.document;
				if (window.HTMLElement && el instanceof window.HTMLElement) return true;
			...
			*/
			closestParent: function(el, selector, includeSelf, doc) {
				...
				while (parent && parent != doc.body) if (parent.matches && parent.matches(selector)) return parent;
			...
			*/
			all: function(tasks, libraries) {
				...
				else if (($ = window.jQuery || libraries.$) && typeof $.when === "function") return $.when.apply($, tasks).done(function() {
					return arguments;
				});
			...
			/** @lends mixitup.Control */
			{
				...
				*/
				update: function(command, toggleArray) {
					...
					if (self.status === "live") self.updateLive(command, toggleArray);
					else {
					...
					for (i = 0; button = controlButtons[i]; i++) {
						...
						switch (self.type) {
							...
							case "multimix":
								...
								actions.sort = button.getAttribute("data-sort");
								break;
							case "toggle": actions.filter = button.getAttribute("data-toggle");
						}
			...
			/** @lends mixitup.Mixer */
			{
				...
				initControls: function() {
					...
					switch (self.config.controls.scope) {
						...
						case "global":
							...
							break;
						default: throw new Error(mixitup.messages.errorConfigInvalidControlsScope());
					}
				...
				*/
				getControl: function(el, type, selector) {
					...
					if (!selector) {
						for (i = 0; control = mixitup.controls[i]; i++) if (control.el === el && control.isBound(self)) return self.callFilters("controlGetControl", null, arguments);
				...
				*/
				getNextSibling: function(index, sibling, position) {
					...
					if (sibling && position === "before") element = sibling;
					else if (sibling && position === "after") element = sibling.nextElementSibling || null;
					else if (self.targets.length > 0 && typeof index !== "undefined") element = index < self.targets.length || !self.targets.length ? self.targets[index].dom.el : self.targets[self.targets.length - 1].dom.el.nextElementSibling;
				...
				*/
				filterOperation: function(operation) {
					...
					for (i = 0; target = operation.newOrder[i]; i++) {
						if (operation.newFilter.collection) testResult = operation.newFilter.collection.indexOf(target.dom.el) > -1;
						else if (operation.newFilter.selector === "") testResult = false;
				...
				*/
				sortOperation: function(operation) {
					...
					} else if (operation.newSort.order === "random") operation.newOrder = h.arrayShuffle(operation.startOrder);
				...
				*/
				getAttributeValue: function(target, attribute) {
					...
					if (value === null) {
						if (self.config.debug.showWarnings) console.warn(mixitup.messages.warningInconsistentSortingAttributes({ attribute: "data-" + attribute }));
					}
				...
				*/
				goMix: function(shouldAnimate, operation) {
					...
					if (!self.userDeferred) deferred = self.userDeferred = h.defer(mixitup.libraries);
					else deferred = self.userDeferred;
					self.isBusy = true;
				...
				*/
				getStartMixData: function(operation) {
					var self = this, parentStyle = window.getComputedStyle(self.dom.parent), parentRect = self.dom.parent.getBoundingClientRect(), target = null, data = {}, i = -1;
					self.incPadding = parentStyle[mixitup.features.boxSizingProp] === "border-box";
					self.callActions("beforeGetStartMixData", arguments);
					for (i = 0; target = operation.show[i]; i++) {
						data = target.getPosData();
						operation.showPosData[i] = { startPosData: data };
					}
					for (i = 0; target = operation.toHide[i]; i++) {
						data = target.getPosData();
						operation.toHidePosData[i] = { startPosData: data };
					}
				...
				*/
				willTransition: function(statusChange, hasEffect, posIn, posOut) {
					...
					if (!h.isVisible(self.dom.container)) result = false;
					else if (statusChange !== "none" && hasEffect || posIn.x !== posOut.x || posIn.y !== posOut.y) result = true;
					else if (self.config.animation.animateResizeTargets) result = posIn.width !== posOut.width || posIn.height !== posOut.height || posIn.marginRight !== posOut.marginRight || posIn.marginTop !== posOut.marginTop;
					else result = false;
				...
				*/
				parseFilterArgs: function(args) {
					...
					for (i = 0; i < args.length; i++) {
						...
						else if (typeof arg === "object" && h.isElement(arg, self.dom.document)) instruction.command.collection = [arg];
						else if (typeof arg === "object" && typeof arg.length !== "undefined") instruction.command.collection = h.arrayFromList(arg);
						else if (typeof arg === "object") h.extend(instruction.command, arg);
						else if (typeof arg === "boolean") instruction.animate = arg;
						else if (typeof arg === "function") instruction.callback = arg;
					}
					...
					for (i = 0; i < args.length; i++) {
						...
						switch (typeof arg) {
							...
							case "boolean":
								...
								break;
							case "function": instruction.callback = arg;
						}
					}
				...
				*/
				parseInsertArgs: function(args) {
					...
					for (i = 0; i < args.length; i++) {
						arg = args[i];
						if (arg === null) continue;
						if (typeof arg === "number") instruction.command.index = arg;
						else if (typeof arg === "string" && ["before", "after"].indexOf(arg) > -1) instruction.command.position = arg;
						else if (typeof arg === "string") instruction.command.collection = h.arrayFromList(h.createElement(arg).childNodes);
						else if (typeof arg === "object" && h.isElement(arg, self.dom.document)) !instruction.command.collection.length ? instruction.command.collection = [arg] : instruction.command.sibling = arg;
						else if (typeof arg === "object" && arg.length) !instruction.command.collection.length ? instruction.command.collection = arg : instruction.command.sibling = arg[0];
						else if (typeof arg === "object" && arg.childNodes && arg.childNodes.length) !instruction.command.collection.length ? instruction.command.collection = h.arrayFromList(arg.childNodes) : instruction.command.sibling = arg.childNodes[0];
						else if (typeof arg === "object") h.extend(instruction.command, arg);
						else if (typeof arg === "boolean") instruction.animate = arg;
						else if (typeof arg === "function") instruction.callback = arg;
					}
					...
					for (i = 0; i < args.length; i++) {
						...
						switch (typeof arg) {
							...
							case "object":
								...
								else if (h.isElement(arg, self.dom.document)) instruction.command.collection = [arg];
								else h.extend(instruction.command, arg);
								break;
							case "boolean":
								instruction.animate = arg;
								break;
					...
					if (self.config.animation.queue && self.queue.length < self.config.animation.queueLimit) {
						...
						if (self.config.controls.enable) {
							if (self.isToggling) {
								...
								self.updateControls({ filter: { selector: toggleSelector } });
				...
				*/
				diffDatasets: function(operation) {
					...
					for (i = 0; data = operation.newDataset[i]; i++) {
						if (typeof (id = data[self.config.data.uidKey]) === "undefined" || id.toString().length < 1) throw new TypeError(mixitup.messages.errorDatasetInvalidUidKey({ uidKey: self.config.data.uidKey }));
						if (!uids[id]) uids[id] = true;
						else throw new Error(mixitup.messages.errorDatasetDuplicateUid({ uid: id }));
				...
				*/
				filter: function() {
					...
					return self.multimix({ filter: instruction.command }, instruction.animate, instruction.callback);
				},
				/**
				...
				*/
				toggleOn: function() {
					...
					toggleSelector = self.getToggleSelector();
					return self.multimix({ filter: toggleSelector }, instruction.animate, instruction.callback);
				},
				/**
				...
				*/
				sort: function() {
					...
					return self.multimix({ sort: instruction.command }, instruction.animate, instruction.callback);
				},
				/**
				...
				*/
				changeLayout: function() {
					...
					return self.multimix({ changeLayout: instruction.command }, instruction.animate, instruction.callback);
				},
				/**
				...
				*/
				insert: function() {
					...
					return self.multimix({ insert: args.command }, args.animate, args.callback);
				},
				/**
				...
				*/
				remove: function() {
					...
					return self.multimix({ remove: args.command }, args.animate, args.callback);
				},
				/**
			...
			*/
			init: function(el, mixer, data) {
				...
				if (data && mixer.config.data.uidKey) {
					if (typeof (id = data[mixer.config.data.uidKey]) === "undefined" || id.toString().length < 1) throw new TypeError(mixitup.messages.errorDatasetInvalidUidKey({ uidKey: mixer.config.data.uidKey }));
					self.id = id;
					self.data = data;
					mixer.cache[id] = self;
				}
			...
			applyStylesOut: function(moveData) {
				...
				switch (moveData.statusChange) {
					case "hide":
						...
						break;
					case "show": isFading && (self.dom.el.style.opacity = 1);
				}
			...
			*/
			eventBus: function(e) {
				...
				switch (e.type) {
					...
					case "transitionend": self.handleTransitionEnd(e);
				}
		...
		*/
		...
		if (typeof exports === "object" && typeof module === "object") module.exports = mixitup;
		else if (typeof define === "function" && define.amd) define(function() {
			return mixitup;
		});
		else if (typeof window.mixitup === "undefined" || typeof window.mixitup !== "function") window.mixitup = mixitup;
